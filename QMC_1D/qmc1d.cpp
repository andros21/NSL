...
double external_potential(double val)
{
	...
	// return k_elastic*val*val/2.0;
	   return pow(val, 2)*(pow(val, 2) - 2.5);
}
...
double variationalWaveFunction(double v)
{
	   double mu(0.786);
           double sigma(0.432);
	   return 1.0;
	// return (exp(mu*v/pow(sigma, 2)) + 1)*exp(-1.0/4.0*(pow(mu, 2) + 2*mu*v + pow(v, 2))/pow(sigma, 2));
}
...
double variationalWaveFunction_second(double v)
{
	   double mu(0.786);
           double sigma(0.432);
	   return 0;
	// return (1.0/4.0)*(pow(mu, 2)*exp(mu*v/pow(sigma, 2)) + pow(mu, 2) - 2*mu*v*exp(mu*v/pow(sigma, 2)) + 2*mu*v - 2*pow(sigma, 2)*exp(mu*v/pow(sigma, 2)) - 2*pow(sigma, 2) + pow(v, 2)*exp(mu*v/pow(sigma, 2)) + pow(v, 2))*exp(-1.0/4.0*(pow(mu, 2) + 2*mu*v + pow(v, 2))/pow(sigma, 2))/pow(sigma, 4);
}
